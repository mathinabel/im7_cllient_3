...
public class PullExtendLayout extends LinearLayout implements IPullToExtend {
     ...
     * 回滚的时间
     */
    private static final int SCROLL_DURATION = 100;//不能太大，太大的话有抖动现象，也不能为0，否则会显示异常
    /**
     * 阻尼系数
     */
    private float offsetRadio = 10.0f;
    ...
    private boolean dealWithMainView(float deltaY){
        ...
        if(mScrollRefreshableView!=null){
            ...
            if(getScrollYValue()==0){
             ...
             /*   if(mScrollRefreshableView instanceof NestedScrollView){
                    NestedScrollView nestedScrollView= (NestedScrollView) mScrollRefreshableView;
                        //注意进行方向的判定
                      return false;
